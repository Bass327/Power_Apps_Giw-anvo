...
import {
  ...
  REGIONS_SENEGAL, REGION_PREFIX_MAP,
...
/**
 * Génère un code projet unique pour une région et une année données.
 * Format : {PREFIX}-{ANNÉE}-{NNN} (ex: DK-2026-001, KLD-2026-009)
 * Cherche le plus grand numéro de séquence existant pour ce préfixe+année
 * et retourne le suivant (3 chiffres avec zéro-remplissage).
 */
function generateCodeProjet(region: string, year: number, existingCodes: string[]): string {
  const prefix  = REGION_PREFIX_MAP[region] ?? region.slice(0, 3).toUpperCase()
...
export default function ProjetFormPage() {
  ...
  // Indique si le code projet affiché a été généré automatiquement (vs saisi manuellement)
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <TextInput value={formData.codeProjet}
                onChange={(v) => {
                  update("codeProjet", v.toUpperCase())
                  // L'utilisateur modifie manuellement → désactiver la régénération automatique
                  setIsCodeAutoGenerated(false)
                }}
                placeholder="KLD-2026-001" error={errors.codeProjet} icon={Hash} />
              {isCodeAutoGenerated
                ? <FieldHint>✦ Généré automatiquement · modifiable</FieldHint>
            ...
            <div>
              ...
              <SelectInput value={formData.region}
                onChange={(v) => {
                  update("region", v)
                  // En mode création : génère le code si le champ est vide ou auto-généré
                  if (!isEdit && v && (isCodeAutoGenerated || !formData.codeProjet)) {
                    const code = generateCodeProjet(
                      v,
                      new Date().getFullYear(),
                      projets.map((p) => p.codeProjet),
                    )
                    update("codeProjet", code)
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between gap-3 mt-5">
            ...
            <div className="flex items-center gap-3">
              {/* Suivant (uniquement si pas à la dernière étape) */}
              {step < STEPS.length && (
              ...
              {/* Créer / Enregistrer — disponible sur toutes les étapes */}
